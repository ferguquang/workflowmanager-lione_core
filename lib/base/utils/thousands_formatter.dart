...
class ThousandsFormatter extends NumberInputFormatter {
  ...
  ThousandsFormatter(
      {this.formatter, this.allowFraction = false, this.decimalDigits = 1})
  ...
  String _formatPattern(String digits) {
    ...
    if (allowFraction) {
      ...
      number = int.tryParse(digits, radix: 2) ?? 0;
    }
    String result = (formatter ?? _formatter).format(number);
    if (result.contains(".") &&
        result.length - result.lastIndexOf(".") + 1 > decimalDigits) {
      int lastIndex = result.lastIndexOf(".") + decimalDigits + 1;
      if (lastIndex < result.length)
        result =
            result.substring(0, result.lastIndexOf(".") + decimalDigits + 1);
    }
  ...
  TextEditingValue formatEditUpdate(
    ...
    if (!result.endsWith(".") &&
        ...
        result.length - result.lastIndexOf(".") + 1 > decimalDigits) {
      int lastIndex = result.lastIndexOf(".") + decimalDigits + 1;
      if (lastIndex < newValue.text.length) {
        result =
            result.substring(0, result.lastIndexOf(".") + decimalDigits + 1);
